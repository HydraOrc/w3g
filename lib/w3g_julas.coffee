		...
		while data_left > @max_datablock
			...
			switch block_id
				...
				when 0
					...
					return exit('Unhandled replay command block at '+convert.time(@time)+': 0x'+php.sprintf('%02X', block_id)+' (prev: 0x'+php.sprintf('%02X', prev)+', time: '+@time+') in '+@filename)
			...
			while n < block_length
				...
				switch action
					...
					when 0x75
						...
						@errors.push 'Unknown action at '+convert.time(@time)+': 0x'+php.sprintf('%02X', action)+', prev: 0x'+php.sprintf('%02X', prev)+', dump: '+temp
